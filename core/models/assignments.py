...
class Assignment(db.Model):
    ...
    def upsert(cls, assignment_new: "Assignment"):
        assertions.assert_valid(
            assignment_new.content is not None, "Content field cannot be empty."
        )
    ...
    def submit(cls, _id, teacher_id, auth_principal: AuthPrincipal):
        ...
        assertions.assert_valid(
            assignment.state is not AssignmentStateEnum.SUBMITTED,
            "only a draft assignment should be submitted",
        )
        assertions.assert_valid(
    ...
    def mark_grade(cls, _id, grade, auth_principal: AuthPrincipal):
        ...
        assertions.assert_valid(
            assignment.state is not AssignmentStateEnum.DRAFT,
            "assignment with empty grade cannot be graded",
        )
        if auth_principal.teacher_id:
            assertions.assert_valid(
                assignment.teacher_id is auth_principal.teacher_id,
                "Assignment is submitted to wrong teacher",
            )
